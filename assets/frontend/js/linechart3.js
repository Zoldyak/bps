...
Vue.use(VueApexCharts);
var appreduksi = new Vue({
	el: '#AngkaReduksiShortfall',
	components: {
		apexchartredux: VueApexCharts,
	},
	data: function() {
		return {
			url: 'http://localhost/bps/',
		...
		getdataahh() {
			...
			axios.get(this.url + "Home/getData").then(function(response) {
				...
				if (response.data == null) {
								...
								m.forEach(function(datatahun) {
									...
									n.forEach(function(dataturtahun) {
										...
										arraydata.forEach(function(hasildata) {
											// console.log(hasildata[hasilkey]);
											if (datalabel['label'] == 'Reduksi Shortfall') {
												datareduksi.push(hasildata[hasilkey].toString());
											}
											// this.ks.xdatakesehatan.push(hasildata[hasilkey].toString());
										})
									})
					...
					// console.log(dataahh);
					this.appreduksi.chartOptions = {
						xaxis: {
							categories: lebelahh,
						}
					};
					this.appreduksi.series = [{
						name: 'Reduksi Shortfall',
						data: datareduksi
					}]
				}
			})
...
Vue.use(VueApexCharts);
var appmelek = new Vue({
	el: '#lamasekolahpendidikan',
	components: {
		lamasekolahpendidikan: VueApexCharts,
	},
	data: function() {
		return {
			url: 'http://localhost/bps/',
			...
			chartOptions: {
				...
				colors: ['#ff0000', '#52ff00'],
				fill: {
					type: 'gradient',
					gradient: {
						gradientToColors: ['#d01212', '#42ba08'],
						// inverseColors: true,
						// shadeIntensity: 1,
						opacityFrom: 0.6,
						opacityTo: 0.1,
		...
		getdataahh() {
			...
			axios.get(this.url + "Home/getData").then(function(response) {
				...
				if (response.data == null) {
								...
								m.forEach(function(datatahun) {
									...
									n.forEach(function(dataturtahun) {
										...
										arraydata.forEach(function(hasildata) {
											// console.log(hasildata[hasilkey]);
											if (datalabel['label'] == 'Angka Melek Huruf ( Persen)') {
												datamelek.push(hasildata[hasilkey].toString());
											}
											if (datalabel['label'] == 'Indeks Pendidikan') {
												datapen.push(hasildata[hasilkey].toString());
											}
											// this.ks.xdatakesehatan.push(hasildata[hasilkey].toString());
										})
									})
					...
					// console.log(dataahh);
					this.appmelek.chartOptions = {
						xaxis: {
							categories: lebelahh,
						}
					};
					this.appmelek.series = [{
							name: 'Angka Melek Huruf ( Persen)',
							data: datamelek
						},
						{
							name: 'Indeks Pendidikan ',
							data: datapen
						}
					]
				}
		...
		getdataahh() {
			...
			axios.get(this.url + "Home/getData").then(function(response) {
				...
				if (response.data == null) {
								...
								m.forEach(function(datatahun) {
									...
									n.forEach(function(dataturtahun) {
										...
										arraydata.forEach(function(hasildata) {
											// console.log(hasildata[hasilkey]);
											if (datalabel['label'] == 'Rata-rata Lama Sekolah (tahun)') {
												datalama.push(hasildata[hasilkey].toString());
											}
											// this.ks.xdatakesehatan.push(hasildata[hasilkey].toString());
										})
									})
					...
					// console.log(dataahh);
					this.applama.chartOptions = {
						xaxis: {
							categories: lebelahh,
						}
					};
					this.applama.series = [{
						name: 'Rata-rata Lama Sekolah (tahun)',
						data: datalama
					}, ]
				}
			})
